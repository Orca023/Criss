...
// 配置預設值;
let interface_Function = Interface_file_Monitor;  // Interface_file_Monitor;  // Interface_http_Server;  // Interface_http_Client;
let interface_Function_name_str = "Interface_File_Monitor";  // "Interface_File_Monitor";  // "Interface_http_Server";  // "Interface_http_Client";

// 配置當 interface_Function = Interface_file_Monitor 時的預設值;
let is_monitor = true;  // Boolean;
// let is_Monitor_Concurrent = "";  // "Multi-Threading"; # "Multi-Processes"; // 選擇監聽動作的函數是否並發（多協程、多綫程、多進程）;
let monitor_dir = String(require('path').join(require('path').resolve(".."), "Intermediary"));  // String(require('path').join(String(__dirname), "Intermediary"));  // process.cwd(), path.resolve("../"),  __dirname, __filename;  // 定義一個網站保存路徑變量;
let monitor_file = String(require('path').join(String(monitor_dir), "intermediary_write_C.txt"));  // String(require('path').join(String(__dirname), "Intermediary", "intermediary_write_C.txt"));  // path.dirname(p)，path.basename(p[, ext])，path.extname(p)，path.parse(pathString) 用於接收傳值的媒介文檔 "../temp/intermediary_write_Python.txt";
let output_dir = String(require('path').join(require('path').resolve(".."), "Intermediary"));  // String(require('path').join(String(__dirname), "Intermediary"));  // path.normalize(p)。path.join([path1][, path2][, ...])，path.resolve('main.js') 用於輸出傳值的媒介目錄 "../temp/";
let output_file = String(require('path').join(String(output_dir), "intermediary_write_Nodejs.txt"));  // String(require('path').join(String(__dirname), "Intermediary", "intermediary_write_Nodejs.txt"));  // path.dirname(p)，path.basename(p[, ext])，path.extname(p)，path.parse(pathString) 用於輸出傳值的媒介文檔 "../temp/intermediary_write_Node.txt";
let temp_NodeJS_cache_IO_data_dir = require('path').join(require('path').resolve(".."), "Intermediary");;  // 一個唯一的用於暫存傳入傳出數據的臨時媒介文件夾 "C:\Users\china\AppData\Local\Temp\temp_NodeJS_cache_IO_data\";
...
let Worker_threads_Script_path = "";  // process.argv[1] 配置子綫程運行時脚本參數 Worker_threads_Script_path 的值 new Worker(Worker_threads_Script_path, { eval: true });
let Worker_threads_eval_value = null;  // true 配置子綫程運行時是以脚本形式啓動還是以代碼 eval(code) 的形式啓動的參數 Worker_threads_eval_value 的值 new Worker(Worker_threads_Script_path, { eval: true });
let do_Function_data = do_data;  // function (argument) { return argument; };  // 函數對象字符串，用於接收執行數據處理功能的函數 "do_data";
// let do_Function = null;  // function (argument) { return argument; };  // 函數對象字符串，用於接收執行數據處理功能的函數 "do_data";
let do_Function_name_str_data = "do_data";
...
let writableAll = false;  // writableAll <boolean> 對於 IPC 伺服器，使管道對所有用戶都可寫。預設值: false。
let ipv6Only = false;  // ipv6Only <boolean> 對於 TCP 伺服器，將 ipv6Only 設置為 true 將會禁用雙棧支援，即綁定到主機:: 不會使 0.0.0.0 綁定。預設值: false。
let do_Function_Request = do_Request_Router;  // 用於接收執行對根目錄(/)的 GET 或 POST 請求處理功能的函數 "do_Request_Router";
// let do_Request = null;  // function (argument) { return argument; };  // 函數對象字符串，用於接收執行對根目錄(/)的 GET 或 POST 請求處理功能的函數 "do_Request_Router";
let do_Function_JSON = {
    "do_Request": do_Function_Request.toString(),  // "function() {};" 函數對象字符串，用於接收執行對根目錄(/)的 GET 或 POST 請求處理功能的函數 "do_Request_Router";
};
let do_Function_name_str_Request = "do_Request";
...
// request_Cookie = request_Cookie.split("=")[0].concat("=", new Base64().encode(request_Cookie.split("=")[1]));  // "Session_ID=".concat(new Base64().encode("request_Key->username:password")); "Session_ID=".concat(escape("request_Key->username:password"));
let post_Data_JSON = {
    "Client_say": "Node.js 16.11.0 http.request()",
    "time": String(now_date)
};
let post_Data_String = JSON.stringify(post_Data_JSON); // 使用'querystring'庫的querystring.stringify()函數，將JSON對象轉換為JSON字符串;
// console.log(post_Data_String);
let do_Function_Response = do_Response_Router;  // 函數對象字符串，用於執行對接收到的服務端返回的響應處理功能的函數 "do_Response_Router";
// let do_Response = null;  // function (argument) { return argument; };  // 函數對象字符串，用於執行對接收到的服務端返回的響應處理功能的函數 "do_Response_Router";
// let do_Function_JSON = {
//     "do_Response": do_Function_Response.toString(),  // "function() {};" 函數對象字符串，JSON 對象，用於執行對接收到的服務端返回的響應處理功能的函數 "do_Response_Router";
// };
do_Function_JSON["do_Response"] = do_Function_Response.toString();  // "function() {};" 函數對象字符串，JSON 對象，用於執行對接收到的服務端返回的響應處理功能的函數 "do_Response_Router";
let do_Function_name_str_Response = "do_Response";
                ...
                if (eval('typeof (' + process.argv[i].split("=")[0] + ')' + ' === undefined && ' + process.argv[i].split("=")[0] + ' === undefined')) {
                    ...
                    switch (process.argv[i].split("=")[0]) {
                        case "interface_Function": {
                            interface_Function_name_str = String(process.argv[i].split("=")[1]);  // "File_Monitor";  // "http_Server";  // "http_Client";
                            // "function() {};" 函數對象字符串，用於接收執行數據處理功能的函數 "do_data";
                            // if (Object.prototype.toString.call(do_Function = eval(process.argv[i].split('=')[1])).toLowerCase() === '[object function]') {
                            //     interface_Function = eval(process.argv[i].split('=')[1]);
                            //     interface_Function_name_str = String(process.argv[i].split("=")[1]);  // "Interface_File_Monitor";  // "Interface_http_Server";  // "Interface_http_Client";
                            // } else {
                            //     interface_Function = null;
                            //     interface_Function_name_str = "";
                            // };

                            if (String(process.argv[i].split("=")[1]) === "File_Monitor") {
                                // "function() {};" 函數對象字符串，監聽指定路徑文檔的服務器函數 "file_Monitor";
                                if (Object.prototype.toString.call(Interface_file_Monitor).toLowerCase() === '[object function]') {
                                    interface_Function = Interface_file_Monitor;
                                    interface_Function_name_str = "Interface_File_Monitor";
                                    if (Object.prototype.toString.call(do_data).toLowerCase() === '[object function]') {
                                        do_Function_data = do_data;
                                        do_Function_name_str_data = "do_data";
                                    } else {
                                        do_Function_data = null;
                                        do_Function_name_str_data = "";
                                    };
                                    // console.log("do Function: " + do_Function_name_str_data);
                                } else {
                                    interface_Function = null;
                                    interface_Function_name_str = "";
                                };
                            };
                            if (String(process.argv[i].split("=")[1]) === "http_Server") {
                                // "function() {};" 函數對象字符串，http 服務器函數 "http_Server";
                                if (Object.prototype.toString.call(Interface_http_Server).toLowerCase() === '[object function]') {
                                    interface_Function = Interface_http_Server;
                                    interface_Function_name_str = "Interface_http_Server";
                                    if (Object.prototype.toString.call(do_Request_Router).toLowerCase() === '[object function]') {
                                        do_Function_Request = do_Request_Router;
                                        do_Function_name_str_Request = "do_Request";
                                    } else {
                                        do_Function_Request = null;
                                        do_Function_name_str_Request = "";
                                    };
                                    // console.log("do Function: " + do_Function_name_str_Request);
                                } else {
                                    interface_Function = null;
                                    interface_Function_name_str = "";
                                };
                            };
                            if (String(process.argv[i].split("=")[1]) === "http_Client") {
                                // "function() {};" 函數對象字符串，http 服務器函數 "http_Client";
                                if (Object.prototype.toString.call(Interface_http_Client).toLowerCase() === '[object function]') {
                                    interface_Function = Interface_http_Client;
                                    interface_Function_name_str = "Interface_http_Client";
                                    if (Object.prototype.toString.call(do_Response_Router).toLowerCase() === '[object function]') {
                                        do_Function_Response = do_Response_Router;
                                        do_Function_name_str_Response = "do_Response";
                                    } else {
                                        do_Function_Response = null;
                                        do_Function_name_str_Response = "";
                                    };
                                    // console.log("do Function: " + do_Function_name_str_Response);
                                } else {
                                    interface_Function = null;
                                    interface_Function_name_str = "";
                                };
                        ...
                        case "port": {
                            port = parseInt(process.argv[i].split("=")[1]);  // 10001; 監聽端口;
                            // console.log("listening Port: " + port);
                            break;
                        }
                        case "webPath": {
                        ...
                        case "do_Response": {
                            // "function() {};" 函數對象字符串，用於執行對接收到的服務端返回的響應處理功能的函數 "do_Response";
                            if (Object.prototype.toString.call(do_Response = eval(process.argv[i].split('=')[1])).toLowerCase() === '[object function]') {
                                do_Response = eval(process.argv[i].split('=')[1]);
                            } else {
                                do_Response = null;
